...
filepath = 'c:\\Users\\HP\\Documents\\Medicine-Recommendation-and-Reminder-System\\Medicine_recommender_system\\frontend\\src\\pages\\Register.jsx'

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# We need to replace the form section
# Find the start of the form
form_start_idx = content.find('<form onSubmit={handleSubmit}')
form_end_idx = content.rfind('</form>') + len('</form>')

if form_start_idx == -1 or form_end_idx == -1:
    print("Could not find form tags")
    exit(1)

new_form = """<form onSubmit={handleSubmit} className="relative z-10 space-y-4">
...
print("Successfully patched Register.jsx")
